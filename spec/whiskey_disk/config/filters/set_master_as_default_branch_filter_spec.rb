...
require File.expand_path(File.join(File.dirname(__FILE__), '..', '..', '..', '..', 'lib', 'whiskey_disk', 'config', 'filters', 'set_master_as_default_branch_filter'))

describe 'setting the default branch for a main repo' do
  before do
    @config = WhiskeyDisk::Config.new
    @filter = WhiskeyDisk::Config::SetMasterAsDefaultBranchFilter.new(@config)
  end
