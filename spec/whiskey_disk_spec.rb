...
describe '@whiskey_disk' do
  ...
  describe 'updating the main repository checkout' do
    ...
    it 'captures the current git HEAD ref for the current branch' do
      @whiskey_disk.configuration = @parameters.merge({'branch' => 'production'})
      @whiskey_disk.update_main_repository_checkout
      @whiskey_disk.buffer.should.satisfy {|b| b.include?("git config deploy.previous-sha \`git log -1 --pretty=format:%H\`") }
    end
    ...
    it 'captures the current git last commit ref for the current branch' do
      @whiskey_disk.configuration = @parameters.merge({'branch' => 'production'})
      @whiskey_disk.update_main_repository_checkout
      @whiskey_disk.buffer.should.satisfy {|b| b.include?("git config deploy.previous-commit \`git log -1 --abbrev-commit --pretty=oneline\`") }
    end
